...
const AllAMAccounts: React.FC<AllAMAccountsProps> = ({ 
  data, 
  loading = false,
  refreshTrigger = 0
}) => {
  ...
  const fetchAndApplyPreferences = async () => {
    ...
    try {
      ...
      const allAMAccountsTablePrefs = userPrefJson?.all_am_accounts_table;
      
      let columnModel = allAMAccountsTablePrefs?.columnVisibilityModel;
  ...
  const handleViewDetails = (id: number) => {
    navigate(`/account-management/account/${id}`);
  };
  ...
  const columns: GridColDef[] = useMemo(() => [
    ...
    {
      field: 'licenses_or_units_sold',
      headerName: 'Licenses Sold',
      width: 130,
      type: 'number',
      sortable: false,
    },
