...
const CustomEmailPage: React.FC = () => {
  ...
  const checkUserExists = async () => {
    try {
      const userExist = loginResponse?.custom_email || "";
      setEmailExists(userExist !== "");
      if (userExist) {
        await fetchAllEmails(userExist);
      }
    } catch (error) {
  ...
  const fetchEmails = async (email: string, label: string) => {
    try {
      const response = await listCustomEmails({
...
const resetComposeForm = useCallback(() => {
  ...
  const handleEmailClick = async (email: EmailData) => {
    ...
    setLoadingEmailId(email.id.toString());

    try {
      const response = await viewCustomEmail({
        user_id: loginResponse?.id,
      ...
      const emailData = {
        ...
        to: email.to,
        date: email.date,
        body: cleanEmailBody(response.html || response.text || ''),
  ...
  const sendEmail = async (to: string, subject: string, body: string) => {
    ...
    try {
      ...
      // Refresh email lists
      const email = loginResponse?.custom_email;
      if (email) {
  ...
  const formatDate = (dateString: string) => {
    try {
      ...
      // If the email is from this year
  ...
  const getSenderInfo = (from: string) => {
    try {
      ...
      // If no match, return the whole string as both name and email
      return {
        name: from,
        email: from
      };
  ...
  const generateAvatarColor = (id: number | string): string => {
    try {
      // Convert number to string and ensure we have a valid value
      const idString = String(id || 0);
      // Use a simple hash function for numbers
  ...
  const renderEmailList = ({ emailList, isLoading }: EmailListProps) => (
          ...
          <div className="divide-y divide-gray-200">
            {filterEmails(emailList, searchTerm).map((email) => {
              ...
              return (
                <div
                  key={email.id}
                  className={`flex items-center px-4 py-3 cursor-pointer transition-colors hover:bg-gray-50 ${isSelected ? 'bg-blue-50' : ''
                    }`}
                ...
                >
                  ...
                  <div className="flex items-center space-x-4" onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      checked={isSelected}
                      onCheckedChange={() => handleToggleSelectEmail(email.id.toString())}
                    />
          ...
          <div className="flex-1 bg-white overflow-hidden">
            <Tabs defaultValue="inbox" className="h-full flex flex-col">
              ...
              <TabsContent value="inbox" className="mt-0 flex-1 overflow-hidden">
                {renderEmailList({ emailList: emails, isLoading: isLoadingInbox })}
              </TabsContent>

              <TabsContent value="sent" className="mt-0 flex-1 overflow-hidden">
                {renderEmailList({ emailList: sentEmails, isLoading: isLoadingSent })}
              </TabsContent>

              <TabsContent value="drafts" className="mt-0 flex-1 overflow-hidden">
                {renderEmailList({ emailList: draftEmails, isLoading: isLoadingDrafts })}
              </TabsContent>
            </Tabs>
          </div>
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 border-b pb-4">
                ...
                <div className="flex items-start gap-4">
                  <Avatar className="h-10 w-10 flex-shrink-0" style={{ backgroundColor: generateAvatarColor(selectedEmail.id) }}>
                        ...
                        .map((message) => (
                          <div key={message.id} className="border-t pt-4">
                            <div className="flex items-start gap-3 mb-3">
                              <Avatar
                                size={24}
                                style={{ backgroundColor: generateAvatarColor(message.id) }}
                              >
