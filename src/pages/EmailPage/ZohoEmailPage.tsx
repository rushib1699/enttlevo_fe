...
const ZohoEmailPage: React.FC = () => {
          ...
          <div className="divide-y divide-gray-200">
            {filterEmails(emailList, searchTerm).map((email) => {
              ...
              return (
                <div 
                  key={email.id}
                  className={`flex items-center px-2 sm:px-4 py-3 cursor-pointer transition-colors border-l-4 min-w-0 ${
  ...
  const ComposeEmailModal = () => (
    <Card className={`fixed bottom-0 right-2 sm:right-6 w-full sm:w-96 max-w-96 z-50 shadow-lg ${composeMinimized ? 'h-12' : 'h-auto'} overflow-hidden`}>
      ...
      {!composeMinimized && (
        <CardContent className="p-3 space-y-3 overflow-y-auto max-h-[calc(100vh-200px)]">
          ...
          {!showCcBcc && (
            <div className="text-right">
              <Button variant="link" size="sm" onClick={() => setShowCcBcc(true)}>
                Cc/Bcc
              </Button>
            </div>
          )}
